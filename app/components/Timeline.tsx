...
let localThemeVar: string | undefined;
...
const getMaxHours = (duration: string): number => {
  ...
  if (hourMatches) {
    ...
    const maxDays = dayMatches[2]
      ? parseInt(dayMatches[2])
      : parseInt(dayMatches[1]);
    return maxDays * 4;
  } else if (weekMatches) {
    // Convert weeks to hours (assuming 4 hours per day, 5 days per week)
    const maxWeeks = weekMatches[2]
      ? parseInt(weekMatches[2])
      : parseInt(weekMatches[1]);
    return maxWeeks * 4 * 5;
  }
...
const extractLearningMaterials = (
  subSteps: string[]
  ...
  subSteps.forEach((subStep) => {
    ...
    const resourceMatch = subStep.match(
      /Resource:\s*\[([^\]]+)\]\(([^)]+)\)(?:\s*-\s*(.+))?/
    );
    if (resourceMatch) {
      materials.push({
...
const extractCodeExamples = (
  ...
  subSteps.forEach((subStep) => {
    ...
    if (
      subStep.startsWith("Practice exercise:") ||
      subStep.startsWith("Project component:")
    ) {
      const title = subStep.startsWith("Practice exercise:")
        ? "Practice Exercise"
        : "Project Component";
      const code = subStep
        .replace(/^(Practice exercise:|Project component:)\s*/, "")
        .trim();
...
export function Timeline({ steps }: TimelineProps) {
  ...
  return (
    <div className="relative max-w-5xl mx-auto py-8">
      {/* Vertical line */}
      <div
        className={`absolute left-[10px] sm:left-[120px] top-0 bottom-0 w-0.5 ${filterUtilityClasses(
          "dark:bg-[#dbdbd9]/20 bg-gray-600",
          theme
        )} `}
      />
          ...
          <div key={step.id} className="relative mb-10">
            <div className="flex items-start">
              ...
              <div className="absolute left-[10px] sm:left-[120px] transform -translate-x-1/2 mt-1.5">
                <div
                  className={cn(
                    `w-4 h-4 rounded-full border-2 ${filterUtilityClasses(
                      "dark:border-[#dbdbd9] border-gray-500",
                      theme
                    )}`,
                    expandedSteps.has(step.id)
                      ? `${filterUtilityClasses(
                          "dark:bg-[#dbdbd9] bg-gray-500",
                          theme
                        )}`
                      : `${filterUtilityClasses(
              ...
              <div className="flex-1 pl-8">
                <Collapsible>
                  <CollapsibleTrigger
                    ...
                    className={`flex items-center space-x-2 text-lg font-semibold tracking-tight ${filterUtilityClasses(
                      "dark:text-[#dbdbd9] dark:hover:text-white text-[#0d0d01] hover:text-black",
                      theme
                    )}  w-full text-left`}
                  >
                    ...
                    <div className="mt-4 space-y-4">
                      ...
                      <div className="space-y-3">
                        {step.subSteps.map((subStep, subIndex) => (
                          ...
                          >
                            <span
                              className={`font-medium ${filterUtilityClasses(
                                "dark:text-[#dbdbd9 text-[#0d0d01]",
                                theme
                              )} mt-0.5`}
                            >
        ...
        {modalContent.step && (
          <div className="space-y-6">
            {extractLearningMaterials(modalContent.step.subSteps).length > 0 ? (
              extractLearningMaterials(modalContent.step.subSteps).map(
                (material, index) => (
                  <div key={index} className="bg-[#191a1a] p-4 rounded-md">
                    ...
                    {material.description && (
                      <p className="text-[#dbdbd9]/70 text-sm">
                        {material.description}
                      </p>
                    )}
                  </div>
              ...
              <div className="text-center py-8">
                <p className="text-[#dbdbd9]/60">
                  No learning materials found for this step.
                </p>
                <p className="text-[#dbdbd9]/40 text-sm mt-2">
                  Try checking the links in the step description.
                </p>
              </div>
            )}
          </div>
        ...
        {modalContent.step && (
          <div className="space-y-6">
            {extractCodeExamples(modalContent.step.subSteps).length > 0 ? (
              extractCodeExamples(modalContent.step.subSteps).map(
                (example, index) => (
                  <div
                    key={index}
                    className="bg-[#191a1a] rounded-md overflow-hidden"
                  >
                    <div className="bg-[#202323] px-4 py-2 border-b border-[#dbdbd9]/10">
                      <h4 className="text-[#dbdbd9] font-medium">
                        {example.title}
                      </h4>
                    </div>
                    <div className="p-4">
                      <pre className="text-[#dbdbd9]/90 text-sm whitespace-pre-wrap font-mono">
                        {example.code}
                      </pre>
                    </div>
                  </div>
              ...
              <div className="text-center py-8">
                <p className="text-[#dbdbd9]/60">
                  No code examples found for this step.
                </p>
                <p className="text-[#dbdbd9]/40 text-sm mt-2">
                  Try expanding the step to see all instructions.
                </p>
              </div>
            )}
          </div>
